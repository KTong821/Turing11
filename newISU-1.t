var window : int := Window.Open ("graphics: 600, 400")
...
procedure spawn
    ...
    elsif random_int = 2 then
	Pic.Draw (small, round (c), 20, picMerge)
    elsif random_int = 3 then
	Pic.Draw (double, round (c), 20, picMerge)
    elsif random_int = 4 then
	Pic.Draw (triple, round (c), 20, picMerge)
    elsif random_int = 5 then
	if bird_check < 50 then
	    Pic.Draw (bird1, round (c), 80, picMerge)
	    bird_check += 1
	elsif bird_check > 50 then
	    Pic.Draw (bird2, round (c), 80, picMerge)
	    bird_check += 1
	end if
	if bird_check >= 100 then
	    bird_check := 0
	end if
    end if
end spawn
...
function run : boolean
	...
	if (Time.Elapsed - timea) < 100 then
	    ...
	    Draw.ThickLine (100, 37, 800, 37, 2, black)
	    Draw.ThickLine (0, 37, 66, 37, 2, black)
